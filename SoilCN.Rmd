...
sir <- function(t, x, p) {
  with(as.list(c(x,p)),{
    ...
    dNl <- ADD/CNadd + BD/CNb - DECl/CNl # C Nitrogen litter pool (eq 7)
    dNh <- rh*(DECl/CNh) - (DECh/CNh) # Humus pool N (eq 10)
    dNb <- (1-rh*CNl/CNh)*DECl/CNl + DECh/CNh - BD/CNb - phi # N Microbial biomass pool (eq 12)
    dNplus <- MIN -IMMplus - NIT - LEplus - UPplus # eq 24 CHECK
    dNminus <- NIT - IMMminus - LEminus - UPminus # eq 25 CHECK
    Ctot <- ADD - rrDECl - rrDECh
    Ntot <- ADD/CNadd - LE+ - UP+ - LE- - UP-
    
    list(c(dCl, dCh, dCb, dNl, dNh, dNb, dNplus, dNminus, Ctot, Ntot))
  })
}
...
CNbio <- 11.5 ###Microbial biomass ðC=NÞbio 11.5
CNh <- 22 ###Humus ðC=NÞh 22
...
kh <- 0.0000025
kn <-  0.6
rh <- min(0.25,CNh/CNl) #CHECK THIS
rr <- 0.6
